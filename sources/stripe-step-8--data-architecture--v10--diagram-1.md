...
flowchart TB
    ...
    subgraph ORCH["⚫ Orchestration (Airflow)"]
        ETL["DAGs ETL/ELT<br/> + conversion de change<br/>(taux de référence fixe)"]
    end
    ...
    OLTP_CORE -.->|"<u>CDC/WAL</u><br/>asynchrone continu<br/>Debezium ou PostgreSQL<br/>(réplication logique)"| TOPIC_CDC
    OLTP_CORE -.->|publication événement<br/>accès asynchrone en lecture| TOPIC_APP
    OLTP_ACCESS -.->|"<u>CDC/WAL</u><br/>comme coeur métier"| TOPIC_CDC
    OLTP_COMP -.->|"<u>CDC/WAL</u><br/>comme coeur métier"| TOPIC_CDC
